...
const HighLightSection = ({ media, className = '', SaveButtons }) => {
  ...
  const generalizeVoteCount = (count) => {
    ...
    >
      <div className="opacity-80 mt-[0.3rem] -mb-1">
        {isIMDB ? (
          ...
          <div
            className="text-[0.8rem] md:text-[1-rem] px-1 rounded font-extrabold text-black"
            style={{
      ...
      >
        <span className="text-3xl md:text-5xl">
          ...
          <span className={`${ratingColor} font-bold`}>
        ...
        {voteCount ? (
          <>
            <span className="font-semibold">
              {generalizeVoteCount(voteCount)}
            </span>
            <span> people rated</span>
          </>
        ) : (
          'No rating'
        )}
      ...
      {SaveButtons && <SaveButtons media={media} classNames="relative gap-2" />}
    </div>
  );
};
